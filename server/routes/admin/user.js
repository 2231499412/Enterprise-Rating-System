...
router.get('/list', authMiddleware(USER_TYPES.ADMIN), async (req, res) => {
  try {
    const { name, userId, status, startDate, endDate } = req.query;

    // Compatible with both {page, pageSize} and {current, size}
    const rawPage = req.query.page ?? req.query.current;
    const rawPageSize = req.query.pageSize ?? req.query.size;
